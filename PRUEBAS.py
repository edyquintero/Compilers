...
def expresion():
    if tokens[currentTokenPos][0] in ['IDENTIFICADOR', 'ENTERO', 'REAL']:
        match(tokens[currentTokenPos][0])
    else:
        raise SyntaxError(
            f"Expresión incorrecta en fila {tokens[currentTokenPos][2]} columna {tokens[currentTokenPos][3]}")


input_string = 'INICIO ASIGNAR A x = 5 SI x == 5 ENTONCES IMPRIMIR 5 FIN SI FIN'
tokens = tokenizar(input_string)
